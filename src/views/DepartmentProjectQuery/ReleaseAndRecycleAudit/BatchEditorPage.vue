  ...
  <div v-if="visible">
    ...
    >
      <div class="batch-editor_page">
        <el-tabs v-model="activeName" class="el-tabs_reset">
          <el-tab-pane label="批次信息" name="1">
            <BatchInfoList :projectCodes="projectCodes" ref="BatchInfoList" :formData="formData" :readonly="readonly" />
          </el-tab-pane>
          <el-tab-pane label="凭证信息" name="2" v-if="routeName==='ReleaseAndRecycleBatch'">
            <VoucherInfoList />
          </el-tab-pane>
          <el-tab-pane label="附件" name="3">
            <AttachmentList ref="AttachmentList" :readonly="readonly" />
          </el-tab-pane>
          <el-tab-pane label="流转信息" name="4" v-if="formData.state!=='Draft'">
            <MoveInfoList />
          </el-tab-pane>
        </el-tabs>
        <el-form :inline="true">
      ...
      <template slot="ms-foot">
        ...
        <template v-if="routeName==='ReleaseAndRecycleCheck'">
          <el-button type="primary" @click="goPass">通 过</el-button>
          <el-button type="danger" @click="callBack">退 回</el-button>
          <el-button @click="closeDialog" icon="el-icon-circle-close">取 消</el-button>
        </template>
...
import BatchInfoList from './components/BatchInfoList.vue'
